...
library(ggrepel)
library(plyr)
colors <- c("#ff0d54",  "#5f2054", "#2f2054", "#0f2054")
# layer,tracer,workload,event,freq,total_event_cost,cost_per_event
data <- read.table("./io-overhead/data/cpu-workload-exits-cost.csv", header=T, sep=",")
...
data_filtered <- ddply(data, .(layer, tracer, workload, event),
                       transform,
                       total_event_cost_avg = median(total_event_cost)
)

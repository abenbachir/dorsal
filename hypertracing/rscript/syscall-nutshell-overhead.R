...
colors <- c("Baseline" = "gray10", "Guest Tracing" = "#6f2054", "Write to disk"="dodgerblue2",
            ...
            "Syscall Probe"="darkorange2", "Time+Compression"="yellow2", "Nested Virt."="#3faf1f")
...
# data = subset(data, layer == 'L2')
data = subset(data, tracer != 'Strace')
data = subset(data, !startsWith(as.character(tracer), 'HyperCompres'))
...
plot <- ggplot(data, aes(x=reorder(tracer, total_time), y = time)) +
  ...
  # scale_y_continuous(breaks = seq(0, max(data$total_time), 50)) +
  ...
  labs(title = "Micro-Benchmarks Tracing system call [10M getcpu syscall]", x ="Tracers", y ="Latency (ns)", fill="Cost of") +

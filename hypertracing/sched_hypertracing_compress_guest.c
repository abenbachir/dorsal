...
#define MAX_EVENT_COMPRESSION 3
...
static unsigned int sched_switch_counter = 0;
...
{
    unsigned long tsc = ktime_to_ns(ktime_get());
    sched_switch_counter++;
    if (sched_switch_counter > MAX_EVENT_COMPRESSION)
    {
	    do_hypercall(0x01, prev->pid, prev->tgid, next->pid,next->tgid);
	    sched_switch_counter = 0;
    }
}

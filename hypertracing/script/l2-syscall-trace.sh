...
start_host_tracing() {
    ...
    lttng enable-channel -k --subbuf-size=8K --num-subbuf=128 vm_channel
    lttng enable-event -k "kvm_x86_hypercall" -c vm_channel
#    lttng enable-event -k "kvm_x86_hypercall,kvm_x86_exit,kvm_x86_entry,kvm_x86_nested*" -c vm_channel
    lttng start
}
stop_host_tracing() {
    lttng stop
...
hypertracing_compress()
{
    workload=$1
    CMD=$2
    output="$trace_dir/hypertracing/l2-syscall_benchmark-hyperT_compress_2-${workload}-$(dbus-uuidgen)"
...
#    head -n 3 /sys/kernel/debug/tracing/trace'"
    ...
    stop_host_tracing
    ssh root@$target_l1 "ssh $target_l2 'rm results.txt; rm trace.dat*'"
}

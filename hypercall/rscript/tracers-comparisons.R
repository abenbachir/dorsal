...
colors <- c("#5f2054", "#ff0d54", "#f04d54","#f18d54")
event1 <- 'hypercall'
...
data2$mode <- event1
# lttng27_2 <- data2
# lttng27_2$elapsed_time <- lttng27_2$elapsed_time + 133
# data2 <- rbind(data2, lttng27_2)
...
unique_data <- unique(data[c("tracer","mode","median", "elapsed_time")])
...
unique_data <- unique(unique_data[c("tracer","mode","median", "overhead")])

p <- ggplot(unique_data, aes(x=tracer, y=median, fill=factor(tracer))) +

...
def main(argv):
    ...
    for event in traces.events:
        ...
        is_kernelspace = fields['nr'] == KERNELSPACE_HYPERCALL_NR
        # function_name = get_kernel_symbol_name(function_address)
        # if "idle" in function_name:
        #     continue
        # Handle entry
        if is_entry:
        ...
        # Handle exit
            ...
            if is_kernelspace:
                ...
                event_json['name'] = program_symbols.get_symbol_name(function_address)
    ...
    trace_events.append({
        'pid': USERSPACE_HYPERCALL_NR,
        'tid': 0,
        'name': "process_labels",
        'ph': 'M',
        'cat': "__metadata",
        'args': {'labels': "User space"},
    })

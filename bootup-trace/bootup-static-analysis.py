...
def main(file):
    path = "/media/abder/New Volume/lttng-traces/full-bootup-tracing-ubuntu-4-11"
    kernel_symbols_path = os.path.join(path, "kernel/mapping/kallsyms.map")
    data_pr_cpu = {}

    # Create TraceCollection and add trace:
    # hash_table = HashTable(kernel_symbols_path)
    kernel_symbols = Symbols(kernel_symbols_path)
    process_list = Process(os.path.join(path, "kernel/mapping/process.txt"))
...
if __name__ == "__main__":
    f = open('static-analysis.csv', 'w')
    f.write("function,exit_time,duration,depth,process,parent,stack\n")
    try:
        main(f)
    except Exception as ex:
        f.close()
